...
async function loadProducts() {
  ...
  data.forEach((item) => {
    ...
    newCard
      ...
      .addEventListener("click", (event) => {
        ...
        const cartProductAmountContainer = document.querySelector(
        ...
        newDiv
          ...
          .addEventListener("click", (event) => {
            cardTotalProducts = Number(
              newCard.querySelector(".item-amount-counter").dataset.amount
            );
            itemAmountCache -= cardTotalProducts;
            yourCartItems.textContent = `Your Cart (${itemAmountCache})`;

            cardTotalValue = newDiv
              .querySelector(".product-in-cart-value-total")
              .textContent.slice(1);
            ...
            newCard.querySelector(".item-amount-container").style.display =
              "none";
            ...
            newCard.querySelector(
              ".add-to-cart-button"
            ).parentElement.style.borderColor = "hsl(14, 65%, 9%)";
          });
      });
    ...
    const orderConfirmedCardWrapper = document.querySelector(
      ".order-confirmed-cards-wrapper"
    );
    const orderConfirmedWrapper = document.querySelector(
    ...
    newCard
      ...
      .addEventListener("click", (event) => {
        ...
        const cartProductAmount = document.querySelector(
          "#cart-product-amount"
        );

        const cardIndex = [...cartProductAmount.children].findIndex(
          (card) =>
            card.querySelector(".product-in-cart-title").innerHTML === item.name
        );

        /* changes the amount value of the in cart item */
        cartProductAmount.children[cardIndex].querySelector(
          ".product-in-cart-amount"
        ).textContent = `${
          event.target.parentElement.querySelector(".item-amount-counter")
            .dataset.amount
        }x`;
        ...
        ).textContent = `$${itemAmount.dataset.amount * item.price}`;
      });

    /* decrease the amount of products */
    newCard
      .querySelector(".decreaseAmount")
      .addEventListener("click", (event) => {
        itemAmountCache--;
        itemAmount.dataset.amount--;
        itemAmount.textContent = itemAmount.dataset.amount;
  ...
  setTimeout(() => {
    ...
    const cardsContainer = document.getElementById("cards-container");
    const cardsImage = cardsContainer.querySelectorAll(
  ...
  confirmOrderButton.addEventListener("click", () => {
    ...
    Array.from(cartProductAmount.children).forEach((card) => {
      ...
      <div style="display: flex;">
        <img src="${
          card.querySelector(".product-in-cart-title").dataset.thumbnailimage
        }">
        <div class="order-confirmed-card-description">
            <p class="order-confirmed-card-title">${
              card.querySelector(".product-in-cart-title").textContent
            }</p>
            <div style="display: flex; gap: 10px;">
              <p class="order-confirmed-card-amount">${
                card.querySelector(".product-in-cart-amount").textContent
              }</p>
              <p class="order-confirmed-card-price">${
                card.querySelector(".product-in-cart-value").textContent
              }</p>
            </div>
        </div>
      </div>
      <p class="order-confirmed-card-total">${
        card.querySelector(".product-in-cart-value-total").textContent
      }</p>
    `;
      orderConfirmedCardWrapper.appendChild(newCard);
      const orderConfirmedTotalValue = document.querySelector(
